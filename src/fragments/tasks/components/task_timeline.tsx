...
const TaskDetails = () => {
	...
	useEffect(() => {
		if (data?.alloted_to_id) {
			...
			const tempItems = data.alloted_to_id.map((user, index) => ({
				...
				group: user,
				title: `Item ${index + 1}`,
				start_time: moment().add(index, 'hours').toDate(),
				end_time: moment().add(index + 1, 'hours').toDate(),
			}));
			setItems(tempItems);
		}
	}, [data]);
	...
	return (
		<div className="max-h-screen overflow-y-scroll bg-gray-100">
			...
			<div className="bg-white p-3 border-b-2 border font-open-sans items-center flex justify-between items-center sticky top-0 z-[100]">
