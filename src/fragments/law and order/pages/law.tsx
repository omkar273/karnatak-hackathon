...
const LawOrderPage = () => {
  const tableStyles = "bg-white p-4 rounded-lg shadow-lg mb-8";

  const renderTrafficManagementColumns = [
    {
      title: "View",
      dataIndex: "Name",
      key: "Name",
    ...
    },
    {
      ...
      render: (data: { type: string; description: string }[]) => (
        <div>
    ...
    },
    {
      ...
      key: "road_closures",
      render: (data: { location: string; status: string }[]) => (
        <div>
          {data.map((value, index) => (
            <div key={index}>
  ...
  return (
    <div className="max-h-screen overflow-y-scroll overflow-hidden bg-gray-100 p-4 space-y-8">
      ...
      <div className={tableStyles}>
        ...
        <Table
          dataSource={law_and_order_data.traffic_management}
          columns={renderTrafficManagementColumns}
          pagination={{
            total: law_and_order_data.traffic_management.length,
      ...
      <div className={tableStyles}>
        <p className="font-bold text-xl mb-4">Public Events Management</p>
        <Table
