...
const StationStaffList: React.FC<Props> = ({ stationId }) => {
  const { loading, error, staff } = useGetAllStaff({
    ...
    posts: [
      RanksEnum.Inspector,
      RanksEnum.SubInpector,
      RanksEnum.HeadConstable,
      RanksEnum.Inspector,
    ],
  });
  ...
  return (
    <div className="w-full p-4 bg-white">
      ...
      <h2>All the police personnel in station are listed here</h2>
      ...
      <div className="w-full flex gap-3 items-center">
        ...
        <Select onValueChange={(s) => setFilter(s)}>
          ...
          <SelectContent>
            ...
            <SelectItem value={RanksEnum.Inspector}>Inspector</SelectItem>
            <SelectItem value={RanksEnum.SubInpector}>Sub Inspector</SelectItem>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 py-8">
        {staffList.map((staff, index) => (
          <StaffInfoCard key={index} user={staff} />
        ))}
      </div>
      {/* user data cards */}
      <div></div>
