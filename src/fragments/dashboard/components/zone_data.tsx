    ...
    return (
        <div>
            <h1 className="font-bold text-3xl px-4 py-1">
                {`${userdata.zone_name} zone (${zoneData?.ZoneCode})`}
            </h1>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-x-5 md:gap-y-3 lg:grid-cols-4 p-4">
                <DataCard
                    title="Crime rate"
                ...
                <DataCard
                    title="Crime rate"
                    value={loading ? '--' : (`${zoneData?.crime_rate} %` ?? '--')}
                    change_percentage={'-36%'}
                />
            </div>
        </div>
